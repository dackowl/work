...
class BuyController extends Controller {
        ...
        if($goods_num_key){
            ...
            if (!$redis->hGet("{$this->user_queue_key}", $this->user_id)) {
                ...
                if ($cartlist==0) {
                    ...
                    $this->ajaxReturn(array("status" => "1",'msg'=>"成功加入队列"));
                }else{
    ...
    public function save(){
        $sidArr = M('shop')->field('s_id')->select();    //查找商品id
        $keyArr = [];
        $redis = $this->connectRedis();
        foreach ($sidArr as $key => $value) {
            $nowk = "goods_".$value['s_id']."_user";
            $mykeys = $redis->hKeys($nowk);
            if (!empty($mykeys)) {
                $myvals = $redis->hVals($nowk);
                foreach ($myvals as $k => $val) {
                    ...
                    try{
                        ...
                        //删掉已存入数据库的hash数据
                        $delRe = $redis->hDel($nowk,$val['user_id']);
