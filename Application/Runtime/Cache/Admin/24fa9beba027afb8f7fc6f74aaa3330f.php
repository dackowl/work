...
<body ng-app="App" ng-controller="check">
	<div class="x-nav">
         <div class="row">
			<div class="col-lg-4"><span >当前位置  : / 目的地管理  / 目的地查看</span></div>
...
<div class="container">
	<div class="row">
		<div class="col-lg-1">
			<button type="button" class="btn btn-info btn-lg add" data-toggle="modal" data-target="#myModal">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body text-center" >
			...
			<div class="input-group">
			  ...
			  <input type="text" class="form-control" aria-label="..." id="city" name="city" ng-model="pro.d_city">
			</div><br>
			<div class="input-group">
			  <span class="input-group-btn">
			...
			<div class="input-group">
			  ...
			  <input type="text" class="form-control" aria-label="..." id="man" name="man" ng-model="pro.d_man">
			</div><br>
			<div class="input-group" >
			      <span class="input-group-btn">
			        <button class="btn btn-default" type="button">状态:</button>
			      </span>
		      	  <span class="form-control"><?php echo ($Sta["s_name"]); ?></span>
			   </div><br>
			<span>介绍:</span><br>
      ...
      <div class="modal-footer">
        ...
        <button type="button"  ng-click="sub()" class="btn btn-primary">提交</button>
      </div>
    </div>
  </div>
</div>
			  ...
			  <select class="form-control" aria-label="..." id="area" name="area" ng-model="pro.d_area">
			   ...
			   <div class="input-group">
		      	...
		      	<select class="form-control" aria-label="..." id="d_st" name="d_st" ng-model='pro.d_st'>
					<?php if(is_array($St)): foreach($St as $key=>$vs): ?><option value='<?php echo ($vs["s_id"]); ?>'><?php echo ($vs["s_name"]); ?></option><?php endforeach; endif; ?>  
			    </select>
			   </div><br>
	         <span>介绍:</span><br>
...
<table class="container layui-table">
      ...
      <tbody>
      <?php if(is_array($Des)): foreach($Des as $key=>$v): ?><tr>
              ...
              <td class="td-status text-center">
                <?php if(is_array($St)): foreach($St as $key=>$vs): if(($vs["s_id"]) == $v["d_st"]): ?><span  class="layui-btn layui-btn-normal layui-btn-mini text-center">
				        	<?php echo ($vs["s_name"]); ?>
              ...
              <td class="td-manage text-center">
                  <a  title="编辑" href="javascript:;" class="ml-5" style="text-decoration:none">
                      <i class="layui-icon check" data-toggle="modal" data-target="#mendModal">&#xe642;</i>
                  </a>
...
console.log($resetst);
$del_url="<?php echo U('Destin/delOder');?>";
$mend_url="<?php echo U('Destin/mend');?>";
$check_url="<?php echo U('Destin/check');?>";
...
angular.module('App', []).controller('check', function($scope, $http) {
	$scope.pro={
